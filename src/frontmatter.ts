 ...
 *
 * `getFrontmatter` from `myst-transforms`, called in `./myst`, copies the first
 * heading it finds into `title`, at any depth and anywhere in the document. It
 * removes that heading from the content only when it is a leading H1; otherwise
 * it sets `content_includes_title`. Showing `title` in that case renders the
 * heading a second time, always as an H1 whatever its original level.
 */
